...
class TbRedactorJS extends CInputWidget
{
	...
	{
		$this->getController()->module->bootstrap->registerAssetCss('redactor.css');
		$this->getController()->module->bootstrap->registerAssetJs('redactor.min.js');

		// Prepend language file to scripts package.
		if ($this->editorOptions['lang'] != 'en')
			$this->getController()->module->bootstrap->registerAssetJs('locales/redactor/'.$this->editorOptions['lang'].'.js');

		if (isset($this->editorOptions['plugins']))
		{
			foreach($this->editorOptions['plugins'] as $name)
			{
				$this->getController()->module->bootstrap->registerAssetCss('redactor/plugins/'.$name.'.css');
				$this->getController()->module->bootstrap->registerAssetJs('redactor/plugins/'.$name.'.js');
			}
		}
		$this->getController()->module->bootstrap->registerRedactor($this->selector, $this->editorOptions);
	}
}
