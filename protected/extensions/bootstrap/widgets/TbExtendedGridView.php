...
class TbExtendedGridView extends TbGridView
{
	...
	public function registerCustomClientScript()
	{
		...
		if ($this->fixedHeader)
		{
			$this->getController()->module->bootstrap->registerAssetJs('jquery.stickytableheaders' . (!YII_DEBUG ? '.min' : '') . '.js');
		...
		if ($this->sortableRows)
		{
			...
			$this->getController()->module->bootstrap->registerAssetJs('jquery.sortable.gridview.js');
		...
		if ($this->selectableCells)
		{
			...
			$cs->registerCoreScript('jquery.ui');
			$this->getController()->module->bootstrap->registerAssetJs('jquery.selectable.gridview.js');
...
class TbPercentOfTypeEasyPieOperation extends TbPercentOfTypeOperation
{
	...
	{
		$this->getController()->module->bootstrap->registerAssetCss('easy-pie-chart.css');
		$this->getController()->module->bootstrap->registerAssetJs('jquery.easy.pie.chart.js');
