...
class TbDatePicker extends CInputWidget
{
	...
	{
        $this->getController()->module->bootstrap->registerAssetCss('bootstrap-datepicker.css');
        $this->getController()->module->bootstrap->registerAssetJs('bootstrap.datepicker.js');
	}
	...
	public function registerLanguageScript()
	{
		if (isset($this->options['language']) && $this->options['language'] != 'en')
		{
			...
			if (@file_exists(Yii::getPathOfAlias('bootstrap.assets').'/js/'.$file))
                $this->getController()->module->bootstrap->registerAssetJs('locales/bootstrap-datepicker.'.$this->options['language'].'.js', CClientScript::POS_END);
		}
	}
}
