...
class XUploadAction extends CAction {
    ...
    public function init( ) {
        ...
        if (!empty($_GET['object_id']))
        {
            ...
            $this->_subfolder = $_GET['object_id'];
        }
    ...
    protected function handleDeleting()
    {
        if (isset($_GET["_method"]) && $_GET["_method"] == "delete") {
            $file = File::model()->findByPk($_GET['file']);

            $success = FALSE;
            if ($file instanceof File)
            {
                $this->_subfolder = $file->object_id;
                $data = array(
                    'path' => $this->path.'/'.$this->_subfolder.'/'.$file->filename
                );
                if ($this->fileExists($data))
                {
                    $success = $this->deleteFile($data);
                }
                $file->delete();
            }
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->save()) {
                ...
                //echo $model->filename;
                $model->{$this->fileAttribute}->saveAs($path . $model->filename);
